...
.. note::
    ...
    ``dmri-commit``, ``trimeshpy``, ``bctpy``, ``bz2file``, ``formulaic``,
    ``numba-kdtree``, ``PyMCubes``) are bundled directly into the scilpy
    conda package at build time. No extra channels or manual pip steps are
    required.
